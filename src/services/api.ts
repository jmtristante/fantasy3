...
export async function apiGet<T>(path: string, _retry = false): Promise<T> {
...
async function apiFetch(url: string, options: RequestInit, _retry = false): Promise<Response> {
  const token = useAuthStore.getState().getBearerToken();
  const headers = {
  ...
  if (res.status === 401 && !_retry) {
    try {
      await useAuthStore.getState().refreshToken();
      const newToken = useAuthStore.getState().getBearerToken();
...
export const fantasyAPI = {
  ...
  acceptOffer: (leagueId: string, marketId: string, offerId: string, offerMoney: number) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/league/${leagueId}/market/${marketId}/offer/${offerId}/accept?x-lang=es`, {
      method: 'POST',
      body: JSON.stringify({ offerMoney }),
    }).then(r => { if (!r.ok) throw new Error('Error'); return r.json(); });
  },
  declineOffer: (leagueId: string, marketId: string, offerId: string) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/league/${leagueId}/market/${marketId}/offer/${offerId}/reject?x-lang=es`, {
      method: 'POST',
    }).then(r => { if (!r.ok) throw new Error('Error'); return r.json(); });
  },
  makeBid: (leagueId: string, marketId: string, money: number) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/league/${leagueId}/market/${marketId}/bid?x-lang=es`, {
      method: 'POST',
      body: JSON.stringify({ money }),
    }).then(r => { if (!r.ok) throw new Error('Error'); return r.json(); });
  },
  modifyBid: (leagueId: string, marketId: string, bidId: string, money: number) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/league/${leagueId}/market/${marketId}/bid/${bidId}?x-lang=es`, {
      method: 'PUT',
      body: JSON.stringify({ money }),
    }).then(r => { if (!r.ok) throw new Error('Error'); return r.json(); });
  },
  cancelBid: (leagueId: string, marketId: string, bidId: string) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/league/${leagueId}/market/${marketId}/bid/${bidId}/cancel?x-lang=es`, {
      method: 'DELETE',
    }).then(r => { if (!r.ok) throw new Error('Error'); return r.json(); });
  },
  payBuyoutClause: (leagueId: string, playerTeamId: string, money: number) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/league/${leagueId}/market/player/${playerTeamId}/clause?x-lang=es`, {
      method: 'PUT',
      body: JSON.stringify({ money }),
    }).then(r => { if (!r.ok) throw new Error('Error'); return r.json(); });
  },
  ...
  updateLineup: (teamId: string, lineupData: any) => {
    return apiFetch(`${BASE_URL}/api/v1/competition/1/teams/${teamId}/lineup?x-lang=es`, {
      method: 'PUT',
  ...
  getMatchStats: (week: number) => {
    return apiFetch(`${BASE_URL}/stats/v1/competition/1/stats/week/${week}?x-lang=es`, {})
