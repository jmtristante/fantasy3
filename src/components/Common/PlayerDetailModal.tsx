...
export default function PlayerDetailModal({ isOpen, onClose, player }: PlayerDetailModalProps) {
  ...
  const dailyChange = useMemo(() => {
    const { datos } = filteredSeriePrecios;
    if (datos.length < 2) return null;
    return datos[datos.length - 1] - datos[datos.length - 2];
            ...
            <div className="space-y-3 overflow-visible">
              ...
              {filteredSeriePrecios.fechas.length === 0 ? (
                ...
                <>
                  ...
                  />
                  {dailyChange != null && (
                    <div className={`flex items-center justify-center gap-1.5 py-1.5 rounded-lg ${dailyChange > 0 ? 'bg-green-50 dark:bg-green-900/20' : dailyChange < 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-gray-50 dark:bg-gray-800'}`}>
                      <span className="text-[10px] text-gray-500">Hoy:</span>
                      <span className={`text-[11px] font-bold tabular-nums ${dailyChange > 0 ? 'text-green-600 dark:text-green-400' : dailyChange < 0 ? 'text-red-500 dark:text-red-400' : 'text-gray-500'}`}>
